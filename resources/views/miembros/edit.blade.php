...
<div class="container">
  <div class="card">
    <div class="card-header">
      <h2>{{ __('global.edit') }} {{ __('cruds.miembro.title_singular') }}</h2>
    </div>
    <div class="card-body">
      <form action="{{ route("miembros.update", $miembro->id) }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <input id="nombre" name="nombre" type="text" class="form-control" tabindex="1" value="{{ old('nombre', isset($miembro) ? $miembro->nombre : '') }}">    
        </div>
        <div class="mb-3">
          <label for="email" class="form-label">{{ __('cruds.miembro.fields.email') }}</label>
          <input id="email" name="email" type="email" step="any" class="form-control" tabindex="2" value="{{ old('email', isset($miembro) ? $miembro->email : '') }}">
        </div>
        <div class="mb-3">
          <label for="fono" class="form-label">{{ __('cruds.miembro.fields.phone') }}</label>
          <input id="fono" name="fono" type="text" step="any" class="form-control" tabindex="3" value="{{ old('fono', isset($miembro) ? $miembro->fono : '') }}">
        </div>
        <div class="mb-3">
          ...
          <select id="region_id" name="region_id" class="form-select" tabindex="4">
            ...
            @foreach($regiones->get() as $index => $region)
              <option value="{{ $index }}" {{ isset($miembro) && ($miembro->region_id == $index) ? 'selected' : ''  }}>
                {{ $region }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <select id="comuna_id" name="comuna_id" class="form-select" tabindex="5">
            <option value="{{ $miembro->comuna_id }}" selected>{{ $miembro->comuna->nombre }}</option>
          </select>
        </div>
        <div class="mb-3">
          <label for="direccion" class="form-label">{{ __('cruds.miembro.fields.address') }}</label>
          <input id="direccion" name="direccion" type="text" class="form-control" tabindex="6" value="{{ old('direccion', isset($miembro) ? $miembro->direccion : '') }}">
        </div>
        <div class="mb-3">
          <label for="nacimiento" class="form-label">{{ __('cruds.miembro.fields.birth') }}</label>
          <div class="input-group">
            <input type="text" id="nacimiento" name="nacimiento" class="form-control date" tabindex="7" value="{{ old('nacimiento', isset($miembro) ? $miembro->nacimiento : '') }}">
            <div class="input-group-text"><i class="far fa-calendar-alt"></i></div>
          </div>
        </div>
        <div class="mb-3">
          <label for="invitado" class="form-label">{{ __('cruds.miembro.fields.invited_by') }}</label>
          <input id="invitado" name="invitado" type="text" step="any" class="form-control" tabindex="8" value="{{ old('invitado', isset($miembro) ? $miembro->invitado : '') }}">
        </div>
        <div class="mb-3">
          <label for="peticion" class="form-label">{{ __('cruds.miembro.fields.petition') }}</label>
          <textarea id="peticion" name="peticion" class="form-control" rows="5" tabindex="9">{{ old('peticion', isset($miembro) ? $miembro->peticion : '') }}</textarea>
        </div>
        <div class="mb-3">
